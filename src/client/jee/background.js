...
(function(){
    ...
    function setDefaultSuggestion(text) {
        if (text) {
            chrome.omnibox.setDefaultSuggestion({"description":"<url><match>JavaEE API Search</match></url> " + text});
        } else {
            chrome.omnibox.setDefaultSuggestion({"description":"<url><match>JavaEE API Search</match></url>"});
        }
    };
    
    // Prefetch necessary data
    var java6_ee_api_ = null;
    chrome.omnibox.onInputStarted.addListener(function(){
        console.log("Input started");
        setDefaultSuggestion('');
        if (localStorage.hasUnexpired('java6_ee_api')) {
            java6_ee_api_ = localStorage.getObject('java6_ee_api');
        } else {
            xhr("http://docs.oracle.com/javaee/6/api/allclasses-frame.html",
            function(url, req) {
                ...
                for (var i = 0; i < matches.length; i++) {
                    ...
                    java6_ee_api_.push({"name":content, "fqn":fqn, "url":href, "type":type});
                }
                localStorage.setObject('java6_ee_api', java6_ee_api_);
            },
            function (url, req) {
    ...
    chrome.omnibox.onInputChanged.addListener(function(text, suggest_callback) {
        ...
        var third = [];
        if (java6_ee_api_) {
            for (var i = 0; i < java6_ee_api_.length; ++i) {
                var item = java6_ee_api_[i];
                var fqn = item["fqn"];
                var url = "http://docs.oracle.com/javaee/6/api/" + item["url"];
                var name = item["name"];
                var type = item["type"];
    ...
    chrome.omnibox.onInputEntered.addListener(function(text) {
        ...
        if (!text) {
            nav("http://docs.oracle.com/javaee/6/api/");
            return;
        }
        ...
        if (stripped_text.endsWith(google_codesearch_suffix)) {
            ...
            nav("http://code.google.com/codesearch#search/&q=" + encodeURIComponent(newquery + " lang:java"));
            return;
        }
        ...
        nav("http://www.google.com/search?q=" + encodeURIComponent("Java EE"+stripped_text));
    });
})();