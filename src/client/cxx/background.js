...
(function(){
    ...
    chrome.omnibox.onInputStarted.addListener(function(){
        ...
        if (localStorage.hasUnexpired('cplusplus_headers')) {
            cplusplus_headers_ = localStorage.getObject('cplusplusheaders');
        } else {
            ...
            function(url, req) {
                ...
                for (var i = 0; i < matches.length; ++i) {
                    ...
                    var href = match.substring(hrefstartidx, hrefendidx);
                    var content = match.substring(contentstartidx, contentendidx);
                    cplusplus_headers_.push({'name':content, 'url':'http://cplusplus.com'+href});
                }
                localStorage.setObject('cplusplus_headers', cplusplus_headers_);
            },
            function(url, req) {
                console.log("Failed to receive: "+url);
            }).send(null);
