...
(function(){
    ...
    function setDefaultSuggestion(text) {
        if (text) {
            chrome.omnibox.setDefaultSuggestion({"description":"<url><match>App Engine™ API Search</match></url> " + text});
        } else {
            chrome.omnibox.setDefaultSuggestion({"description":"<url><match>App Engine™ API Search</match></url>"});
        }
    };
    ...
    chrome.omnibox.onInputStarted.addListener(function(){
        ...
        if (localStorage.hasUnexpired('gae_api')) {
            gae_api_ = localStorage.getObject('gae_api');
        } else {
            xhr("http://code.google.com/appengine/docs/java/javadoc/allclasses-frame.html",
            function(url, req) {
                ...
                for (var i = 0; i < matches.length; i++) {
                    ...
                    gae_api_.push({"name":content, "fqn":fqn, "url":href, "type":type});
                }
                localStorage.setObject('gae_api', gae_api_);
            },
            function (url, req) {
    ...
    chrome.omnibox.onInputChanged.addListener(function(text, suggest_callback) {
        ...
        if (gae_api_) {
            for (var i = 0; i < gae_api_.length; ++i) {
                var item = gae_api_[i];
                var fqn = item["fqn"];
                var url = "http://code.google.com/appengine/docs/java/javadoc/" + item["url"];
                var name = item["name"];
                var type = item["type"];
        ...
        if (stripped_text.length >= 2) {
            suggestions.push({"content":stripped_text +  " [Google Code Search]", 
                "description":["Search for \"<dim>appengine</dim> <match>", stripped_text, "</match> <dim>lang:java</dim>\" using <match><url>Google Code Search</url></match> - <url>http://code.google.com/codesearch#search/&amp;q=", encodeURIComponent("appengine " + stripped_text + " lang:java"), "</url>"].join('')}); 
            suggestions.push({"content":stripped_text +  " [Development and Coding Search]", 
                "description":["Search for \"<dim>appengine</dim> <match>", stripped_text, "</match>\" using <match><url>Develoment and Coding Search</url></match> - <url>http://www.google.com/cse?cx=005154715738920500810:fmizctlroiw&amp;q=", encodeURIComponent("appengine "+stripped_text), "</url>"].join('')});
        }
    ...
    chrome.omnibox.onInputEntered.addListener(function(text) {
        ...
        if (!text) {
            nav("http://code.google.com/appengine/docs/java/javadoc/");
            return;
        }
        ...
        if (stripped_text.endsWith(google_codesearch_suffix)) {
            ...
            nav("http://code.google.com/codesearch#search/&q=" + encodeURIComponent("appengine "+newquery + " lang:java"));
            return;
        }
        ...
        if (stripped_text.endsWith(devsearch_suffix)) {
            ...
            nav("http://www.google.com/cse?cx=005154715738920500810:fmizctlroiw&q=" + encodeURIComponent("appengine "+newquery));
            return;
        }
        ...
        nav("http://www.google.com/search?q=" + encodeURIComponent("Google App Engine "+stripped_text));
    });
})();