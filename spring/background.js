...
(function(){
    ...
    function setDefaultSuggestion(text) {
        if (text) {
            chrome.omnibox.setDefaultSuggestion({"description":"<url><match>Spring API Search</match></url> " + text});
        } else {
            chrome.omnibox.setDefaultSuggestion({"description":"<url><match>Spring API Search</match></url>"});
        }
    };
    
    // Prefetch necessary data
    var spring_api_ = null;
    chrome.omnibox.onInputStarted.addListener(function(){
        console.log("Input started");
        setDefaultSuggestion('');
        if (localStorage.hasUnexpired('spring_api')) {
            spring_api_ = localStorage.getObject('spring_api');
        } else {
            xhr("http://static.springsource.org/spring/docs/current/javadoc-api/allclasses-frame.html",
            function(url, req) {
                ...
                for (var i = 0; i < matches.length; i++) {
                    ...
                    spring_api_.push({"name":content, "fqn":fqn, "url":href, "type":type});
                }
                localStorage.setObject('spring_api', spring_api_);
            },
            function (url, req) {
    ...
    chrome.omnibox.onInputChanged.addListener(function(text, suggest_callback) {
        ...
        var third = [];
        if (spring_api_) {
            for (var i = 0; i < spring_api_.length; ++i) {
                var item = spring_api_[i];
                var fqn = item["fqn"];
                var url = "http://static.springsource.org/spring/docs/current/javadoc-api/" + item["url"];
                var name = item["name"];
                var type = item["type"];
        ...
        if (stripped_text.length >= 2) {
            suggestions.push({"content":stripped_text +  " [Google Code Search]", 
                "description":["Search for \"<dim>spring</dim> <match>", stripped_text, "</match> <dim>lang:java</dim>\" using <match><url>Google Code Search</url></match> - <url>http://code.google.com/codesearch#search/&amp;q=", encodeURIComponent("spring " + stripped_text + " lang:java"), "</url>"].join('')}); 
            suggestions.push({"content":stripped_text +  " [Development and Coding Search]", 
                "description":["Search for \"<dim>spring</dim> <match>", stripped_text, "</match>\" using <match><url>Develoment and Coding Search</url></match> - <url>http://www.google.com/cse?cx=005154715738920500810:fmizctlroiw&amp;q=", encodeURIComponent("spring " + stripped_text), "</url>"].join('')});
        }
    ...
    chrome.omnibox.onInputEntered.addListener(function(text) {
        ...
        if (!text) {
            nav("http://static.springsource.org/spring/docs/current/javadoc-api/");
            return;
        }
        ...
        if (stripped_text.endsWith(google_codesearch_suffix)) {
            ...
            nav("http://code.google.com/codesearch#search/&q=" + encodeURIComponent("spring " + newquery + " lang:java"));
            return;
        }
        ...
        if (stripped_text.endsWith(devsearch_suffix)) {
            ...
            nav("http://www.google.com/cse?cx=005154715738920500810:fmizctlroiw&q=" + encodeURIComponent("spring "+newquery));
            return;
        }
        ...
        nav("http://www.google.com/search?q=" + encodeURIComponent(stripped_text + " site:springsource.org"));
    });
})();